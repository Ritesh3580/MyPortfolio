...
function PricingSection({ items }) {
  return (
    <section className="section-shell py-14 sm:py-20 lg:py-24" id="pricing">
      ...
      <div className="mt-8 sm:mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <GlassCard key={item.name} className={`relative flex h-full flex-col p-8 ${item.featured ? 'border border-cyan-300/28 shadow-[0_0_80px_rgba(100,144,255,0.16)]' : ''}`}>
            ...
            <div className="mt-auto pt-8 text-3xl font-semibold text-white">Contact for quote</div>
            <Button href="https://wa.me/918077176860?text=Hello%20Ritesh%2C%20I%20want%20a%20quote%20for%20my%20project." target="_blank" rel="noopener noreferrer" variant={item.featured ? 'primary' : 'secondary'} className="mt-5">
              Discuss Package
            </Button>
          </GlassCard>
        ))}
