...
function ProblemSection({ items }) {
  return (
    <section className="section-shell py-14 sm:py-20 lg:py-24">
      ...
      <div className="mt-8 sm:mt-12 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
        {items.map((item, index) => (
          <GlassCard key={item} className="p-6">
            <div className="flex items-start gap-4">
              <div className="royal-gradient flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-sm font-black text-white">{String(index + 1).padStart(2, '0')}</div>
              <div>
